...
_CHILD_SYSTEM: dict[str, str] = {
    ...
    "donor": (
        "あなたはNPO法人姫路YMCAの寄付者向けインパクトレポートを作成する専門家です。"
        "子どもたちの体験が持つ教育的・社会的意義を、寄付者が共感できる言葉で伝えます。"
        "提供された根拠発言リストのみを根拠に記述します。"
        "子どもの発言は「」で直接引用し、リストに存在しない感情・印象の推測は行いません。"
    ),
}
...
_SUMMARY_SYSTEM: dict[str, str] = {
    ...
    "donor": (
        "あなたはNPO法人姫路YMCAの活動インパクトレポートを作成する専門家です。"
        "寄付者に対して活動の社会的意義と子どもたちの変化を効果的に伝えます。"
        "提供された発言抜粋に存在する事実のみを根拠に記述します。"
    ),
}
    ...
    def generate_session_summary(
        ...
        *,
        ...
        if audience == "parent":
            ...
            child_slot_lines = "\n".join(
                f"{c}は「（{c}の発言抜粋から1つ引用）」と話してくれるなど、（体験の意義を1文で）"
                for c in children
            )
        else:
            child_slot_lines = "\n".join(
