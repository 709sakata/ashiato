...
4.  **切片化 (Stage 1)**: 発言を「指導要録」の3観点に自動分類。児童音声がない（支援者のみ録音）場合も、支援者の発言から学習の証跡を推定（`（支援者の観察）` プレフィックス付きで出力）。
...
```bash
PYTHONPATH=src python3 src/ashiato/usecase/map_speakers.py <文字起こしされた.csv>
# 対話形式で SPEAKER_00 等に「太郎」「山田」などの名前を割り当てます
# → mapped_YYYYMMDD.csv と mapped_YYYYMMDD_meta.txt が生成されます
```
...
- **匿名化**: `src/ashiato/usecase/map_speakers.py` 実行時に、報告書用の児童名を匿名化（児童A, Bなど）するオプションがあります。
