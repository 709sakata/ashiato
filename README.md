...
NPO法人姫路YMCAが運営する「太子遊び冒険の森ASOBO」（兵庫県揖保郡太子町）を活動の場とするフリースクールです。ASOBOは1973年開設の里山型教育キャンプ場で、山・谷・池を有する豊かな自然環境（里山）が広がります。クヌギ・コナラをはじめとする多様な植生と生き物が生息し、自然観察・ハイキング・ボルダリング・アーチェリー・ジップライン・カヌー・昼食調理・火起こし体験・工作など、体験型学習の場として活用されています。

---

## なぜこのツールが必要か

フリースクール「あしあと」のような学校外の居場所では、子どもたちの成長が現場の実感として確かに存在します。しかし、それを「学校の言葉」で記録・共有するコストが、支援の継続的な課題になっています。
...
python3 ~/scripts/ashiato/generate_report.py mapped.csv \
  --date "2025年11月23日" \
  --location "太子遊び冒険の森ASOBO（兵庫県揖保郡太子町）" \
